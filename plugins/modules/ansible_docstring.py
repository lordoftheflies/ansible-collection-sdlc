...
# Copyright: (c) 2019, László Hegedűs <laszlo.hegedus@cherubits.hu>
    ...
    for (dirpath, dirnames, ansible_modules) in walk(path):
        if dirpath == path:
            for mod in ansible_modules:
                try:
                    try:
                        doc, plainexamples, returndocs = plugin_docs.get_docstring(path + mod)
                    except ValueError:
                        doc, plainexamples = plugin_docs.get_docstring(path + mod)
